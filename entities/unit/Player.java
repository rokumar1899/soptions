...
public class Player extends Unit
{
	...
	private void getInput()
	{
    	...
    	if(connector.getKeyManager().getX())
    	{
    		if(equipped.equals("bow"))
    			this.shoot();
    		else 
    			this.meleeAttack();
    			// decrease durability
    	}
    	if(connector.getKeyManager().getZ())
    	{
    		for(Entity e: connector.getWorld().getEntityManager().getEntities())
    		{
    			if(e.equals(this))
    				continue;
    			if(this.getBoundary().intersects(e.getBoundary()))
    			{
    				e.interact();
    			}
    		}
    	}
    	if(connector.getKeyManager().getC())
    	{
    		connector.getGame().crafting();
    	}
	}
	
	public void pickUpDrops(Unit animal)
	{
		if(!animal.isAlive())
		{
			for(Item x: animal.getDrops())
				inventory.add(x);
		}
	}
	...
	public void shoot()
	{
		...
		if(this.getxMove()<0)
		{
			Arrow arrow = new Arrow(this.getXloc()-this.getWidth(), this.getYloc());
			shooting.moveArrowL();
		}
		...
		{
			Arrow arrow= new Arrow(this.getXloc(), this.getYloc()+this.getHeight());
			shooting.moveArrowU();
		}
		else if (this.getyMove()>0)
		{
		...
		arrow.setArrowCount(arrow.getArrowCount-1);
	}
	
	public void meleeAttack()
	{
		int xpos = (int) (this.getXloc()+this.getWidth());
		int xwidth = this.getWidth();
		int ypos = (int) (this.getYloc());
		int xheight = this.getHeight();
		Rectangle attackBounds = new Rectangle(xpos, ypos, xwidth, xheight);
		for(Entity e : connector.getWorld().getEntityManager().getEntities())
		{
			if(e.equals(this))
				continue;
			else if(attackBounds.intersects(e.getBoundary()) && this.getCanAttack())
			{
				e.setHealth((int) (e.getHealth()-super.getDamage()));
			}
		}
	}
	...
	public void generateDrops() {return;}
}
